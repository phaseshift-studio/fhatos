...
namespace fhatos {
  ...
  void test_thread_spawn() {
    ...
    FOS_TEST_OBJ_EQUAL(Obj::to_noobj(),
                       PROCESS("*abc")); // should be noobj else loop() is being pre-executed (i.e. block failing)
    int counter = 0;
    FOS_TEST_OBJ_NTEQL(Obj::to_noobj(), PROCESS("/sys/scheduler::spawn(@z)")); // prev: test thread structure pre-spawn;
