...
namespace fhatos {
    ...
    void test_str() {
     ...
     void test_uri() {
        Types::singleton()->saveType(id_p(FOS_TYPE_PREFIX "uri/webpage"), Obj::to_bcode()); //
        Types::singleton()->saveType(id_p(FOS_TYPE_PREFIX "uri/ftp"), Obj::to_bcode()); //
        ...
        FOS_TEST_OBJ_EQUAL(uriA->split(fURI("//abc/123"), FOS_TYPE_PREFIX "uri/webpage"), uriB->split(fURI("//abc/123"), "webpage"));
        FOS_TEST_OBJ_EQUAL(uriA->split(fURI("//abc/123"), FOS_TYPE_PREFIX "uri/webpage"), uriB->split(fURI("//abc/123"), FOS_TYPE_PREFIX "uri/webpage"));
        ...
        FOS_TEST_OBJ_EQUAL(uriA->split(fURI("//abc/xyz"))->as(FOS_TYPE_PREFIX "uri/webpage"), uriB->split(fURI("//abc/123"))->split(fURI("//abc/xyz"))->as("webpage"));
        FOS_TEST_OBJ_NOT_EQUAL(uriA->split(fURI("//abc/xyz"))->as("webpage"), uriB->split(fURI("//abc/xyz")));
        FOS_TEST_OBJ_NOT_EQUAL(uriA->split(fURI("//abc/xyz"))->as("webpage"), uriB->split(fURI("//abc/123/NONE"))->as(FOS_TYPE_PREFIX "uri/webpage"));
        /// apply
