...
namespace fhatos {
  ...
  void test_type_check_derived_mono_types() {
    Compiler compiler = Compiler(false,false);
     for(const auto& TYPE_MAKER : List<BiConsumer<string,string>>({
             [](const string id, const string source){  TYPE_SAVER(id_p(id.c_str()),mmadt::Parser::singleton()->parse(source.c_str()));},
  ...
  void test_inst_resolution() {
    ...
    FOS_TEST_FURI_EQUAL(INT_FURI->add_component(MMADT_SCHEME "/plus"), compiler.resolve_inst(jnt(1),Obj::to_inst({jnt(10)}, id_p("plus")))->tid_->no_query());
    string ds;
   ...
   void test_derived_type_inst_resolution() {
  	FOS_TEST_FURI_EQUAL(ID("/compiler/nat?dom=/mmadt/int&dc=1,1&rng=/mmadt/int&rc=1,1"), *PROCESS("/compiler/nat -> |/compiler/nat?int<=int()[is(gt(0))]")->tid_);
