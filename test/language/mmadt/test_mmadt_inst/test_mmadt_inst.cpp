...
namespace fhatos {
  ...
  void test_as_inst() {
  	FOS_TEST_FURI_EQUAL(ID("/abc/nat?dom=/mmadt/int&dc=1,1&rng=/mmadt/int&rc=0,1"), *PROCESS("/abc/nat -> |/abc/nat?int{?}<=int{1,1}()[is(gt(0))]")->tid_);
  ...
  void test_drop_inst() {
    FOS_TEST_FURI_EQUAL(fURI(MMADT_SCHEME "/from"), *PROCESS("/abc/drop_1 -> |*/abc/drop_2")->tid_);
    FOS_TEST_FURI_EQUAL(fURI(MMADT_SCHEME "/plus"), *PROCESS("/abc/drop_2 -> |plus(10)")->tid_);
