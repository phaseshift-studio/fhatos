...
namespace fhatos {
  ...
  void test_to_inst() {
    FOS_TEST_OBJ_EQUAL(Obj::to_int(5), Obj::to_inst(jnt(5))->apply(jnt(1)));
    FOS_TEST_OBJ_EQUAL(Obj::to_int(5), Obj::to_inst(__().plus(jnt(1)).bcode_)->apply(jnt(4)));
    FOS_TEST_OBJ_EQUAL(
        Obj::to_int(5),
        Obj::to_inst(__().plus(__().from("/abc/a")).bcode_, Obj::to_inst_args({{"/abc/a", jnt(2)}}))->apply(jnt(3)));
  }
  ...
  void test_choose_inst() {
    ...
    FOS_TEST_OBJ_EQUAL(PROCESS("[is(gt(2))=>noobj,is(gt(0))=>1,_=>noobj]"),
                       PROCESS("1-|[is(gt(2))=>_,is(gt(0))=>_,_=>_]"));
    FOS_TEST_OBJ_EQUAL(PROCESS("[is(gt(2))=>noobj,is(gt(0))=>11,_=>noobj]"),
                       PROCESS("1-|[is(gt(2))=>_,is(gt(0))=>plus(10),_=>_]"));
  }
  ...
  void test_plus_inst() {
    ...
    // str
    FOS_TEST_OBJ_EQUAL(objs({str("ax"), str("bx"), str("bx"), str("cdex")}),
                       PROCESS_ALL("{'a','b','b','cde'}.plus('x')"));
