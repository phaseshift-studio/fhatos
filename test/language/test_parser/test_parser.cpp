...
// TODO:    FOS_TEST_ASSERT_EQUAL_FURI(fURI("x@/int/nat"), *i->id());
  ...
  void test_rec_parsing() {
    ...
    for (const string &form: forms) {
      ...
      TEST_ASSERT_EQUAL_STRING("dogturd", rc2->rec_get(Obj::to_uri("name"))->str_value().c_str());
    ...
    TEST_ASSERT_EQUAL(OType::REC, rc2->o_range());
    TEST_ASSERT_EQUAL_INT(13, (rc2->rec_get(share(Str("a"))))->int_value());
    ...
    const ptr<Rec> rc3 = rc2->rec_get(Obj::to_uri("actor@127.0.0.1"));
    TEST_ASSERT_EQUAL(OType::REC, rc3->o_range());
    TEST_ASSERT_EQUAL_INT(1, (rc3->rec_get(share(Str("b"))))->int_value());
    TEST_ASSERT_EQUAL_INT(3, (rc3->rec_get(share(Str("c"))))->int_value());
  ...
  void test_as_parsing() {
    ...
    FOS_CHECK_RESULTS<Int>({Int(32, "/int/even")}, Fluent(parser->parse("__(32).as(/int/even)")), {}, false);
