...
[INFO]  main [total=>11313152,free=>11311776,used=>0.01217]
...
[ERROR] [/mmadt/noobj] boot config file not found: data../../../conf/boot_config.obj
...
[INFO]  [/sys/router/] boot_config.hpp boot config header loaded [size: 1065 bytes]
...
[INFO]  [/boot/config] 
   [
     ...
     setup=>setup()[start({heap[[pattern=>+/#]]@/mnt/var}).end().start({fs[[pattern=>/fs/#,config=>[root=>/fs]]]@/mnt/fs}).end().mount(at(/mnt/var)).end().mount(at(/mnt/fs)).end().start({log[[=>]]@/io/log}).end().start({terminal[[=>]]@/io/terminal}).end().start({console[[=>]]@/io/console}).end().start({/io/console/config/stack_size}).ref(259072).end().start({button[[pin=>gpio[5],on_push=>lift(print('button pushed'))]]@/io/z}).end().spawn(at(/io/console)).end().bundle(at(/io/z)).end()]
   ]@/boot/config
      .oO configuring kernel objs Oo.
[INFO]  [/sys/mem] main [total=>11718656,free=>11716800,used=>0.01584]
...
[INFO]  [/boot/config] /sys/router boot config dropped
[INFO]  [/sys/scheduler] [/sys/scheduler] scheduler started
[INFO]  [/sys/scheduler] scheduler configured
            [id=>/sys/scheduler,config=>[def_stack_size=>32384]]
[INFO]  [/boot/config] /sys/scheduler boot config dropped
...
[INFO]  [/sys/mem] main [total=>11718656,free=>11716800,used=>0.01584]
[INFO]  [/sys/type] /mmadt/proc type defined
[INFO]  [/sys/type] /mmadt/proc/::/eval type defined
[INFO]  [/sys/router/] /io/parser obj loaded
      .oO finalizing boot w/ setup inst Oo.
[INFO]  [/sys/mem] main [total=>12394496,free=>12392480,used=>0.01627]
[INFO]  [/boot/config] exiting boot state: stricter type checking enabled
[INFO]  [/boot/config] applying setup inst
            setup()[start({heap[[pattern=>+/#]]@/mnt/var}).end().start({fs[[pattern=>/fs/#,config=>[root=>/fs]]]@/mnt/fs}).end().mount(at(/mnt/var)).end().mount(at(/mnt/fs)).end().start({log[[=>]]@/io/log}).end().start({terminal[[=>]]@/io/terminal}).end().start({console[[=>]]@/io/console}).end().start({/io/console/config/stack_size}).ref(259072).end().start({button[[pin=>gpio[5],on_push=>lift(print('button pushed'))]]@/io/z}).end().spawn(at(/io/console)).end().bundle(at(/io/z)).end()]
...
[INFO]  [/sys/mem] main [total=>88166400,free=>88164352,used=>0.00232]
...
[INFO]  [/sys/router/] /fs/# fs detached
----
...
[ERROR] [/mmadt/parser] 2 is not a str as defined by [str][_] 
...
error==>true
fhatos> 10.is_divisible('abc')
error[ERROR] [/mmadt/parser] 'abc' is not a int as defined by [int][_] 
		 \_/mmadt/str is not a subtype of /mmadt/int
----
...
==>int8[128]@a
fhatos> @a + 1                               // <3>
[ERROR] [/mmadt/parser] 129@a is not a /mmadt/ext/int8 as defined by uint8()[is(gte(-127)).is(lte(128))] 
		 \_int8[129]@a is not applicable with /mmadt/ext/uint8?dom=/mmadt/int&dc=1,1&rng=/mmadt/int&rc=1,1
...
[ERROR] [/mmadt/parser] noobj is not a nat as defined by nat()[is(gte(0))] 
...
fhatos> a -> age[45]
[ERROR] [/mmadt/parser] 45 is not a age as defined by is(gt(0)).is(lt(120)) 
