...
FhatOS software can be written in C/C\++ or mm-ADT (**multi-model abstract data type**). mm-ADT is a reflective, cluster-oriented programming language with a fluent syntax and an an underlying monoidal structure.
...
. `/mmadt/uri`: The infinite set of all fHatOs UTF-8 Uniform Resource Identifiers (fURIs).
...
mm-ADT is actually a very simple language.
The above line obscures this because it is using numerous https://en.wikipedia.org/wiki/Syntactic_sugar[syntactic sugars].
The "sugar-free" form of the expression above is:
...
<2> Use the `uri` to store a value into an underlying structure. `a = ...`
<3> Fetch the value to store from the `uri`. `a = get(a) ...`
<4> Add to the fetched valued stored at the `uri`. `a = get(a) + ...`.
<5> Add `uri[/mmadt/ext]` to the fetched value. `a = get(a) + b`.

The one instruction that was not mentioned above is `block` (sugar'd `|`).
This is probably the most useful instruction in the whole language.
To understand `block`, it's necessary to first understand how instructions are evaluated.

===== Inst Evaluation Mechanics

An instruction has the following form:

[source,mmadt,role=center]
----
input --> inst(arg1, arg2, ...) --> output
----

When an input `obj` reaches an `inst`, the following mechanical process happens:
...
Note that the application is recursive in that `1.mult(plus(3))` is evaluated as follows:
...
A `rec` behaves in a similar manner to `lst` and `inst` when a left-hand side `obj` is applied to it. However, what makes
...
This feature of `rec` make it both a https://en.wikipedia.org/wiki/Data_structure[data structure] and a https://en.wikipedia.org/wiki/Control_flow[flow control structure] as once an `obj` has been applied to `rec`,
the values of `rec` can be "drained". For instance, `if` is implemented with a two entry `rec`, where one entry maps to `noobj`.

++++
<!-- 🐖 ./main_runner.out
{1,2,3}.[is(gt(2)) => _, _ => noobj]          --- <1>
...
[source,mmadt]
----
fhatos> {1,2,3}.[is(gt(2)) => _, _ => noobj]          // <1>
=>[
==>1=>noobj
=>]
=>[
...
<1> A stream of `objs` is applied one-by-one to the `rec` yielding a new internally-applied `rec`.
<2> The internally-applied `rec` is "drained" via the `merge` `inst` (sugar'd `>-`).

In the above example, since `1` and `2` were mapped to `noobj`, they are effectively removed from the execution pipeline.
However, because `3` is `gt(2)`, it is mapped to `_` (its self). Thus, when `>-` is applied to this `rec`, the result is
`{noobj,noobj,3}` which is equivalent to `{3}`. In this way, `rec` is both a data structure and a flow control structure.

It's not difficult to realize how an "if"-`rec` generalizes to support
"https://en.wikipedia.org/wiki/Conditional_(computer_programming)#If%E2%80%93then(%E2%80%93else)[if-else]",
"https://en.wikipedia.org/wiki/Conditional_(computer_programming)#Case_and_switch_statements[switch]",
"https://en.wikipedia.org/wiki/Conditional_(computer_programming)#Guarded_conditionals[guards]",
"https://en.wikipedia.org/wiki/Conditional_(computer_programming)#Pattern_matching[pattern matching]"
"https://en.wikipedia.org/wiki/Conditional_(computer_programming)#Hash-based_conditionals[hash-conditionals]",
and other variations on the computing's most universal theme: "https://en.wikipedia.org/wiki/Branch_(computer_science)[branch]".


====
NOTE: The `merge` (sugar'd `>-`) instruction has a correlate: `split` (sugar'd `-<`). The way to think of these two
instructions is that they either branch a serial execution pipeline (`split`) or the join a collection of parallel
executing pipelines (`merge`). Interestingly, the application of an `obj` to a `poly` implements the `split` instruction.
So why does an explicit `split` instruction exist? Because there are other ways in which branching pipelines can be defined
and evaluated. This will be discussed later when discussing `fos:thread`, `fos:coroutine`, and `fos:fiber`.
====
