...
It has a few moving parts that drive the entire computation.
Understanding these fundamentals is the key to mastering mm-ADT.
...
These operators parse down to instructions which have a familiar functional form of `f(a,b,c,...)`.
For example, the above expression in a sugar-free form is written below:
...
fhatos> a -> 12
==>12
msg[[target=>a,payload=>12,retain=>true]]fhatos> @a.inspect()
=>[
==>type=>[
===>id=>/mmadt/int
===>obj=>[int][_]
...
=>]
=>]
msg[[target=>a,payload=>12@a,retain=>true]]fhatos> @a.plus(1)
==>13@a
msg[[target=>a,payload=>13@a,retain=>true]]fhatos> @a.plus(1).plus(1)
==>15@a
----
...
fhatos> a -> 'meangirl'            // <5>
==>'meangirl'
[ERROR] [/sys/scheduler] [/mmadt/noobj] noobj accessed as uri
	  thrown at inst msg[[target=>a,payload=>'meangirl',retain=>true]] => to(b) [0=>noobj,1=>true]
fhatos> *b                         // <6>
----
...
==>12
[ERROR] [/sys/scheduler] [/mmadt/noobj] noobj accessed as uri
	  thrown at inst msg[[target=>a,payload=>12,retain=>true]] => to(b) [0=>noobj,1=>true]
fhatos> *a
==>12
fhatos> *b
----
