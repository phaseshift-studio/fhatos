...
[source,mmadt]
----
fhatos> 10.(min=>2,max=>10)[is(and([gte(*min),lte(*max)]))]                // <1>
[ERROR] [/sys/scheduler] noobj [[noobj][_]] not in range of inst(min=>2,max=>8)[is(and([gte(from(min)),lte(from(max))]))] [SIGNATURE HERE]
...
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a int as defined by [int][_]
	  ...
	  thrown at inst 10 => inst()[is(and([gte(from(min)),lte(from(max))]))] [=>]
fhatos> |((min=>2,max=>8)[is(and([gte(*min),lte(*max)]))])                // <4>
==>inst?noobj<=obj(min=>2,max=>8)
	[is(and([gte(from(min)),lte(from(max))]))]
...
thread spawned: lift(from(block(c),0).print('c: {_}').plus(1).to(c).gt(10).to(t/halt).is(_).print('\n')) [stack size:16192]
c: 0
c: 1
c: 2
c: 3
