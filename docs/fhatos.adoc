...
[ERROR] [/mmadt/rec] unable to connect to mqtt://chibi.local:1883: MQTT error [-1]: TCP/TLS connect failure
...
----
fhatos> # -> noobj
[ERROR] [/sys/scheduler] [/mmadt/noobj] noobj accessed as bool
	  thrown at inst # => ref?obj{?}<=obj{?}(true)[cpp] [1=>true]
	  thrown at inst console[[halt=>false,delay=>nat[0],loop=>inst()[cpp],config=>[nest=>2,prompt=>'fhatos> ',strict=>false,log=>INFO,stack_size=>24288,stack_trace=>true]]]@/io/console => eval?obj{?}<=console(code=>'# -> noobj')[cpp] [code=>'# -> noobj']
----
...
image:ansi/punky.png[width=250,float=left,role=title=submarine] mmADT is the programming language of [red]#F#​[fuchsia]#H#​[lime]#a#​[aqua]*T*​[yellow]*O*​[blue]#s#​. In mmADT, every expression is an `obj` (object). The language has an
...
==>char['b']@b
fhatos> @a + @b
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/int as defined by [int][_] 
	  thrown at inst 'aa'@a => char()[merge(2).count().is(eq(1))] [0=>char['a']@a]
...
fhatos> nat[-30]
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/int as defined by [int][_] 
	  thrown at inst -30 => nat()[is(gte(0))] [code=>'nat[-30]']
...
fhatos> @a.minus(2)
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/int as defined by [int][_] 
	  thrown at inst -1@a => nat()[is(gte(0))] [0=>2]
...
==>C[274]
fhatos> C[-274.0]
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/real as defined by [real][_] 
	  thrown at inst -274.000000 => C()[is(gte(-273.149994))] [code=>'C[-274.0]']
...
fhatos> @a + 1                               // <3>
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/int as defined by [int][_] 
	  thrown at inst 129@a => uint8()[is(gte(-127)).is(lte(128))] [0=>1]
...
fhatos> nat[-6]
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/int as defined by [int][_] 
	  thrown at inst -6 => nat()[is(gte(0))] [code=>'nat[-6]']
...
fhatos> nat[3].mult(-2)
[ERROR] [/sys/scheduler] [/sys/lang/parser] noobj is not a /mmadt/int as defined by [int][_] 
	  thrown at inst -6 => nat()[is(gte(0))] [0=>-2]
...
image:ansi/submarine.png[width=250,float=right,role=img-pad,title=submarine] Every structure supports 2 primary operations:
