...
When an mmADT base type is wrapped in a latexmath:[\texttt{type}[\]]-bracket, the type is first resolved to it's `obj` form (typically as an `inst`) and then the `obj` is applied to it.
...
The type can be understood as a predicate, where an `error` or `noobj` is `false`, otherwise `true`.
...
The `@`-reference ensures that as the `obj` mutates, it's corresponding representation in the underlying fURI structure mutates as well.

These concepts are demonstrated in the examples below using the generally useful types off the `/mmadt/ext` prefix.
