...
[INFO]  [/sys/router] ../../../conf/boot_config.obj boot config file loaded (size: 814 bytes)
[INFO]  [/sys/router] 
  [
    ...
    console=>[terminal=>[stdout=>/io/terminal/:stdout,stdin=>/io/terminal/:stdin],nest=>2,ellipsis=>125,prompt=>'fhatos> ',strict=>false,log=>INFO,stack_size=>24288,stack_trace=>true]
    fs=>[root=>./data/fs]
  ]@/boot/config
...
++++
<!-- 🐖 ./main_runner.out
band -> ||band?int{?}<=int(_,_)[is(gte(*<0>)).is(lte(*<1>))]
3.band(2,8)                           --- <1>
10.band(2,8)                          --- <3>
-->
++++
...
==>band?int{?}<=int(_,_)[band?int{?}<=int[is(gte!...
fhatos> 3.band(2,8)                           // <1>
==>3
...
++++
<!-- 🐖 ./main_runner.out
band -> ||band?int{?}<=int(min=>_,max=>_)[is(gte(*min)).is(lte(*max))]
3.band(2,8)                           --- <1>
...
==>band?int{?}<=int(min=>_,max=>_)[band?int{?}<...
fhatos> 3.band(2,8)                           // <1>
==>3
...
[ERROR] [/sys/scheduler] [/mmadt/int] 2 accessed as str
	  thrown at inst 'abc' => gte(from(min)) [min=>2,max=>8,0=>2]
	  thrown at inst 'abc' => band?int{?}<=int(min=>2,max=>8)[band?int{?}<=int[is(gte(from(min))).is(lte(from(max)))]] [min=>2,max=>8]
----
...
++++
<!-- 🐖 ./main_runner.out
band -> ||band?int{?}<=int(min=>else(2),max=>else(8))[is(gte(*min)).is(lte(*max))]
1.band(min=>1)                        --- <1>
10.band()                             --- <2>
...
==>band?int{?}<=int(min=>else?noobj<=obj(2)[noobj],...
...
[ERROR] [/sys/scheduler] [/mmadt/obj] /mmadt/lte?dom=/mmadt/obj&dc=1,1&rng=/mmadt/bool&rc=1,1 inst unresolved 
	  ...
	  thrown at inst 2 => band?int{?}<=int(min=>3,max=>4)[band?int{?}<=int[is(gte(from(min))).is(lte(from(max)))]] [min=>3,max=>4]
----
...
==>is_divisble?bool<=int(by?int=>_)[is_divisble[mod(from(by...
...
==>is_divisble?bool<=int(by?int=>is?noobj<=obj(neq?noobj<=!...
fhatos> 10.is_divisible(2)
error==>true
fhatos> 10.is_divisible('abc')
...
[ERROR] [/sys/scheduler] [/mmadt/int] 0 accessed as uri
	  thrown at inst /type/int/nat => gt(0) [0=>0]
fhatos> nat[6]
==>nat[6]
fhatos> nat[-6]
