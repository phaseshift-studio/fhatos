...
In the code example above, the expression to import `/mmadt/ext` is pretty intense looking, to say the least.
...
The `fos:frame` is the only aspect of an `inst` that makes it unique because every `poly`-type supports the same internally recursive application of an left-hand side `obj`.
...
This feature of `rec` make it both a data structure and a flow control structure as once an `obj` has been applied to `rec`,
the values of `rec` can be "drained". For instance,
