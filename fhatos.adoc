...
mm-ADT is a structurally typed language, whereby if an `obj` *A* __matches__ `obj` *B*, then *A* is _a type of_ *B*. Defining types is as simple writing an `obj` to the FhatOS fURI space. For instance, if a natural number $\mb{N}$ is any non-negative number, then natural numbers are a subset (or refinement) of *int*.

++++
<!-- CODE:BASH:START -->
<!-- ./docs/build/main_runner.out "/int/nat -> |is(gt(0))" "/int/even -> |mod(2).is(eq(0))" "nat[20]" "nat[-42]" "even[4]" "even[1]" -->
<!-- CODE:END -->
<!-- OUTPUT:START -->
<!-- ⚠️ This content is auto-generated by `markdown-code-runner`. -->
++++
...
[source,mmadt]
----
fhatos> thread[[setup => |0.to(x),
                loop  => |from(x).switch([
                                  is(gt(10)) => stop(/abc/),
                                  _          => plus(1).to(x).print(_)])
               ]].to(/abc/)
==>thread[[setup=>block[start[{0}].to[x]],loop=>block[from[x].switch[[is[gt[10]]=>stop[/abc/],_=>plus[1].to[x].print[_]]]]]]
[INFO]  [/sys/scheduler/] /abc/ thread spawned
1
2
3
4
...
10
11
[INFO]  [/sys/scheduler/] /abc/ thread destroyed
----
++++
<!-- OUTPUT:END -->
++++
...
==>16
fhatos> *b.plus(45)
[ERROR]  noobj[Ø] unexpectedly acccessed for uri_value [line:319]
----
++++
<!-- OUTPUT:END -->
++++
