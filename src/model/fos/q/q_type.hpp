...
namespace fhatos {
  ...
  public:
    ...
    static ptr<QType> create(const ID_p &value_id = nullptr) { return make_shared<QType>(value_id); }
    ...
    void write(const QProc::POSITION pos, const fURI &furi, const Obj_p &obj, const bool retain) override {
      if(string(furi.query()).find("=") == string::npos) {
        Compiler().with_derivation_tree().type_check(obj, furi.query());
      }
    }
    ...
    [[nodiscard]] Obj_p read(const QProc::POSITION pos, const fURI &furi, const Obj_p &post_read) const override {
      if(!post_read->is_noobj() && string(furi.query()).find("=") == string::npos) {
        Compiler().with_derivation_tree().type_check(
            post_read->is_objs() && post_read->objs_value()->size() == 1 ? post_read->objs_value()->front() : post_read,
            furi.query());
      }
      return post_read;
    }

    [[nodiscard]] ON_RESULT is_post_read() const override { return ON_RESULT::IGNORE_Q; }

    [[nodiscard]] ON_RESULT is_pre_write() const override { return ON_RESULT::IGNORE_Q; }
  };
} // namespace fhatos
#endif
