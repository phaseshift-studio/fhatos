/*!
 * @file  MLX90614xx.cpp
 * @brief  MLX90614xx.cpp Initialize the I2C,
 ...
 */
#include "mlx90614xx.hpp"
#ifndef NATIVE
MLX90614xx::MLX90614xx()
{
}

int MLX90614xx::begin(void)
{
  uint8_t idBuf[2];
...
void MLX90614xx::setEmissivityCorrectionCoefficient(float calibrationValue, bool set0X0F)
{
...
void MLX90614xx::setMeasuredParameters(eIIRMode_t IIRMode, eFIRMode_t FIRMode)
{
  uint8_t buf[2] = { 0 };
...
float MLX90614xx::getAmbientTempCelsius(void)
{
  uint8_t buf[2];
...
float MLX90614xx::getObjectTempCelsius(void)
{
  uint8_t buf[2];
...
float MLX90614xx::getObject2TempCelsius(void)
{
  uint8_t buf[2];
...
uint8_t MLX90614xx::readModuleFlags(void)
{
...
MLX90614xx_I2C::MLX90614xx_I2C(uint8_t i2cAddr, TwoWire* pWire, int sdaPin, int sclPin)
{
  _deviceAddr = i2cAddr;
  _pWire = pWire;
...
int MLX90614xx_I2C::begin(void)
{
  ...
  return MLX90614xx::begin();   // Use the initialization function of the parent class
}

void MLX90614xx_I2C::enterSleepMode(bool mode)
{
  if (mode) {
...
void MLX90614xx_I2C::setI2CAddress(uint8_t addr)
{
  uint8_t buf[2] = { 0 };
...
unsigned char MLX90614xx_I2C::crc8Polyomial107(unsigned char* ptr, size_t len)
{
  unsigned char i;
...
void MLX90614xx_I2C::sendCommand(uint8_t cmd)
{
...
void MLX90614xx_I2C::writeReg(uint8_t reg, const void* pBuf)
{
  if (pBuf == NULL) {
...
size_t MLX90614xx_I2C::readReg(uint8_t reg, void* pBuf)
{
  size_t count = 0;
  uint8_t pec = 0;
  if (NULL == pBuf) {
