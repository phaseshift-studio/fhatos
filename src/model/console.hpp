...
namespace fhatos {
  ...
  protected:
    ...
    explicit Console(const ID &value_id, const Rec_p &config) : /* for new line formatting in IDE */
      Thread(id_p(value_id), Obj::to_rec(
               {{":loop", InstBuilder::build(id_p(value_id.extend(":loop")))
                   ...
                   ->inst_f([this](const Obj_p &, const InstArgs &) -> Obj_p {
                     try {
                       // to allow the residual threads to startup
                       static bool first = true;
                       if(first) {
                         first = false;
                         this->delay(500);
                       }
                       if(FOS_IS_DOC_BUILD) return noobj();
                       if(this->new_input_) this->print_prompt(!this->line_.empty());
                       this->new_input_ = false;
                       //// READ INPUT UNTIL \n
                       const string line_read = this->read_stdin('\n')->str_value();
                       this->line_ += line_read;
                       StringHelper::trim(this->line_);
                       if(!this->tracker_.track(line_read)->closed() ||
                          this->line_.empty() ||
