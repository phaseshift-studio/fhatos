...
#define FOS_PWM_ANALOG_RESOLUTION 4095
  ...
  static void ARDUINO_ISR_ATTR ISR_FUNCTION(void *arg) {
    ...
    if (bcode->is_noobj())
      LOG(ERROR, "no interrupt code found for !yISR_FUNCTION!! at %s", code_furi->toString().c_str());
    else
  ...
  protected:
    Pattern read_gpio_pattern_;
    Pattern write_gpio_pattern_;
    Pattern pwm_pattern_;

  protected:
    fURIPinDriver(const Pattern &read_gpio_pattern, const Pattern &write_gpio_pattern, const Pattern &pwm_pattern) :
        PinDriver(), read_gpio_pattern_(read_gpio_pattern), write_gpio_pattern_(write_gpio_pattern),
        pwm_pattern_(pwm_pattern) {}

  public:
    static ptr<fURIPinDriver> create(const Pattern &read_gpio_pattern, const Pattern &write_gpio_pattern,
                                     const Pattern &pwm_pattern) {
      const auto driver = ptr<fURIPinDriver>(new fURIPinDriver(read_gpio_pattern, write_gpio_pattern, pwm_pattern));
      return driver;
    }
    ...
    void digital_write(const uint8_t pin, const bool value) override {
      router()->write(furi_p(this->write_gpio_pattern_.resolve(string("./") + to_string(pin))), dool(value));
    }

    bool digital_read(const uint8_t pin) override {
      const Bool_p b = router()->read(furi_p(this->read_gpio_pattern_.resolve(string("./") + to_string(pin))));
    ...
    uint16_t analog_read(const uint8_t pin) override {
      const Int_p i = router()->read(furi_p(this->pwm_pattern_.resolve(string("./") + to_string(pin))));
