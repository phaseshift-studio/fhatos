...
namespace fhatos {
  struct Insts {
   ...
   return Obj::to_inst("union", {rec}, [rec](const Obj_p &lhs) {
    ...
    static Uri_p type() {
      return Obj::to_inst(
          "type", {}, [](const InstArgs &) { return [](const Obj_p &lhs) { return share(Uri(ID(*lhs->id()))); }; },
          IType::ONE_TO_ONE);
    }
    ...
    static Obj_p pub(const Uri_p &target, const Obj_p &payload, const Bool_p &retain) {
      return Obj::to_inst(
          ...
          [](const InstArgs &args) {
            return [args](const Obj_p &lhs) {
              router()->route_message(share(Message{.target = args.at(0)->apply(lhs)->uri_value(),
            ...
            return [args](const Obj_p &lhs) {
              ...
              if (on_recv_applied->is_noobj()) {
                ...
                router()->route_subscription(share(Subscription{
                    .source = ID("mmadt"), .pattern = pattern_applied->uri_value(), .on_recv = on_recv_applied}));
              }
              return lhs;
            };
          },
  ...
  public:
    static Int_p sum() {
      return Obj::to_inst("sum", {}, reduce_([](const Obj_p &a, const Obj_p &b) { return share(*b + *a); }),
                          IType::MANY_TO_ONE, Obj::objs_seed());
    }

    static Int_p prod() {
      return Obj::to_inst("prod", {}, reduce_([](const Obj_p &a, const Obj_p &b) { return share(*b * *a); }),
                          IType::MANY_TO_ONE, Obj::objs_seed());
    }
          ...
          [](const InstArgs &args) {
            return [args](const Poly_p &lhs) {
              ...
              if (lhs->is_str()) {
                ...
                const Objs_p strs =
                    Options::singleton()->processor<Objs, BCode, Objs>(Obj::to_objs(share(chars)), args.at(0));
                string ret;
    ...
    static BCode_p to_bcode(const Consumer<Message_p> &consumer, const ID &label = ID("cpp-impl")) {
      return bcode({Insts::lambda(
          [consumer](const Rec_p &message) {
            const Message_p mess = share(Message{.target = message->rec_get(uri("target"))->uri_value(),
