  ...
  public:
    ...
    ptr<Bytecode> parseBytecode(stringstream *ss) {
      ...
      while (!ss->eof()) {
        ...
        if (*opcode == "is") {
          range = domain;
          fluent = new Fluent(fluent->is(OBJ_OR_BYTECODE<Bool>::create(args->at(0)->obj)));
        } else if (*opcode == "branch") {
          fluent = new Fluent(fluent->branch(OBJ_OR_BYTECODE<Rec>::create(args->at(0)->obj)));
        } else if (*opcode == "eq") {
          fluent = new Fluent(fluent->eq(OBJ_OR_BYTECODE<Obj>::create(args->at(0)->obj)));
        } else if (*opcode == "plus") {
          range = domain;
          fluent = new Fluent(fluent->plus(OBJ_OR_BYTECODE<Obj>::create(args->at(0)->obj)));
        } else if (*opcode == "mult") {
          range = domain;
          fluent = new Fluent(fluent->mult(OBJ_OR_BYTECODE<Obj>::create(args->at(0)->obj)));
