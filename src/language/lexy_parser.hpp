...
#ifndef fhatos_lexy_parser_hpp
#define fhatos_lexy_parser_hpp
...
namespace grammar
{
    ...
    {
        static constexpr auto rule = [] {
            // We try parsing the colon. This means that a missing colon raises an error, which is
            // then caught and parsing continues as if nothing happens. Without the try, parsing the
            // current item would be canceled immediately.
            ...
            // A (potentially empty) list of items, seperated by comma and surrounded by curly
            // brackets.
...
int main(int argc, char** argv) {
    ...
    auto json = lexy::parse<grammar::json>(file.buffer(),nullptr);
    if (json.has_value())
        json.value().print();
