...
namespace fhatos {
    class Rooter final : public Patterned {
    protected:
        MutexDeque<Structure *> structures = MutexDeque<Structure *>();

        explicit Rooter(const Pattern &pattern) : Patterned(p_p(pattern)) {
        }
                ...
                if (!found->load()) {
                    if (id->matches(*structure->pattern())) {
                        LOG_STRUCTURE(TRACE, this, "writing %s to !b%s!! at " FURI_WRAP " for " FURI_WRAP "\n",
                                      obj->toString().c_str(), id->toString().c_str(),
                                      structure->pattern()->toString().c_str(),
                                      source->toString().c_str());
    ...
    Rooter *rooter() {
        return Options::singleton()->rooter<Rooter>();
    }
} // namespace fhatos
