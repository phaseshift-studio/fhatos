...
namespace fhatos {
    ...
    public:
        ...
        void stop() override {
            ...
            try {
                this->xmqtt->disconnect()->wait();
                if (!this->xmqtt->disconnect()->try_wait()) //wait_for(2000))
                    LOG_STRUCTURE(ERROR, this, "Unable to gracefully disconnect from mqtt broker !g[!y%s!g]!!\n",
                              this->server_addr);
            } catch (const std::exception &e) {
                LOG_STRUCTURE(ERROR, this, "Unable to gracefully disconnect from mqtt broker !g[!y%s!g]!!: %s\n",
                              this->server_addr, e.what());
            }
        }
