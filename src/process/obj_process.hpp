...
namespace fhatos {
  ...
  public:
    explicit ProcessObj(const ID &id) :
      PROCESS(id, rec()), process_rec_(router()->read(make_shared<ID>(id))->clone()),
    ...
    void setup() override {
      try {
        ...
        LOG_PROCESS(DEBUG, this, "Executing setup()-bcode: %s\n",
                    this->process_rec_->rec_get(vri(":setup"))->toString().c_str());
        process(this->process_rec_->rec_get(vri(":setup")), this->id_uri_);
    ...
    void loop() override {
      try {
        if (this->running_.load()) {
          const BCode_p loop_bcode = this->process_rec_->rec_get(vri(":loop"));
          // router()->read(this->id())->rec_get(vri(this->id()->resolve(":loop")));
      ...
      try {
        if (this->running_.load()) {
          LOG_PROCESS(DEBUG, this, "Executing stop()-bcode: %s\n",
                      this->process_rec_->rec_get(vri(":stop"))->toString().c_str());
          process(this->process_rec_->rec_get(vri(":stop")), this->id_uri_);
          PROCESS::stop();
        }
      } catch (const fError &error) {
        LOG_EXCEPTION(error);
  ...
  public:
    explicit ThreadObj(const ID &id) :
      ProcessObj(id) {
    }
  };
  ...
  public:
    explicit CoroutineObj(const ID &id) :
      ProcessObj(id) {
    }
  };
  ...
  /* inline Rec_p load_process(const Process_p &process,
     ...
     record->rec_set(vri(process->id()->extend(":setup")), Obj::to_bcode(
                       [process](const Obj_p &) {
                         process->setup();
                         return noobj();
                       },
     ...
     record->rec_set(vri(process->id()->extend(":stop")), Obj::to_bcode(
                       [process](const Obj_p &) {
                         process->setup();
                         return noobj();
                       },
