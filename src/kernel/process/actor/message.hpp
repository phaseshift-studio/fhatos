...
public:
  ...
  [[nodiscard]] virtual const String toString() const {
    char temp[100];
  ...
  /*template<typename = typename std::enable_if<std::is_base_of_v<int, PAYLOAD>,
  int>>
  ...
  std::enable_if<std::is_base_of_v<float,PAYLOAD>,float>>
  [[nodiscard]]  const String payloadString() const {
      return String() + payload;
  }

  template<typename = typename std::enable_if<std::is_base_of_v<bool,
  PAYLOAD>,bool>>
